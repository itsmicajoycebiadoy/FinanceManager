...
function App() {
  ...
  const [transactions, setTransactions] = useState(() => {
    ...
    if (savedUser) {
      const saved = localStorage.getItem(`transactions_${savedUser}`);
  ...
  const [deletedTransactions, setDeletedTransactions] = useState(() => {
    const savedUser = localStorage.getItem('userName');
    if (savedUser) {
      const saved = localStorage.getItem(`deletedTransactions_${savedUser}`);
      return saved ? JSON.parse(saved) : [];
    }
    return [];
  });
  ...
  // --- UPDATED EXPORT LOGIC FOR MOBILE ---
  const exportToCSV = () => {
    ...
    try {
      ...
      const fileName = `Finance_Report_${userName}_${new Date().toISOString().split('T')[0]}.csv`;
